...
export function NetworkSettingsWindow({ onClose }: NetworkSettingsWindowProps) {
  const [activeTab, setActiveTab] = useState<"general" | "sync" | "storage">("general");
  ...
  return (
    <Win98Window
      title="Network & Offline Settings"
      icon={<span className="text-[12px]">{isOffline ? "📡" : "🌐"}</span>}
      onClose={onClose}
      defaultPosition={{ x: Math.max(40, window.innerWidth / 2 - 180), y: Math.max(40, window.innerHeight / 2 - 220) }}
      defaultSize={{ width: 380, height: 440 }}
      minSize={{ width: 340, height: 380 }}
    >
      <div className="flex flex-col h-full bg-[hsl(var(--win98-face))] text-[11px]">
        {/* Tab bar */}
        <div className="flex border-b border-[hsl(var(--win98-shadow))] px-2 pt-1">
          {tabs.map((tab) => (
            <button
              key={tab.id}
              onClick={() => setActiveTab(tab.id)}
              className={`px-3 py-1 border border-b-0 rounded-t-sm text-[11px] -mb-px ${
                activeTab === tab.id
                  ? "bg-[hsl(var(--win98-face))] border-[hsl(var(--win98-shadow))] z-10 font-bold"
                  : "bg-[hsl(var(--win98-face)/0.7)] border-[hsl(var(--win98-shadow))] text-[hsl(var(--win98-button-text)/0.6)]"
              }`}
            >
              {tab.label}
            </button>
            ...
            <div className="space-y-3">
              ...
              <fieldset className="border border-[hsl(var(--win98-shadow))] p-2 rounded-sm">
                ...
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <div>
                    ...
                    <div className="text-[10px] text-[hsl(var(--win98-button-text)/0.6)]">
                      {isOffline ? "Using cached data" : "Connected to server"}
                    </div>
                  </div>
                </div>
                <div className="flex items-center gap-2 text-[10px] text-[hsl(var(--win98-button-text)/0.7)]">
              ...
              <fieldset className="border border-[hsl(var(--win98-shadow))] p-2 rounded-sm">
                ...
                <div className="space-y-1.5">
                  ...
                  {syncProgress && (
                    <div className="text-[10px] text-[hsl(var(--win98-button-text)/0.6)] flex items-center gap-1">
              ...
              <fieldset className="border border-[hsl(var(--win98-shadow))] p-2 rounded-sm">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-1 text-[10px] text-[hsl(var(--win98-button-text)/0.6)]">
                    <Shield className="h-3 w-3" />
                    AES-256-GCM encryption: {encryptionEnabled ? "Enabled" : "Disabled"}
                  </div>
                </div>
              </fieldset>
            </div>
                        ...
                        return (
                          <tr key={key} className="border-b border-[hsl(var(--win98-shadow)/0.3)]">
                            <td className="py-0.5 pr-2 flex items-center gap-1">
                              <Database className="h-3 w-3 text-[hsl(var(--win98-button-text)/0.5)]" />
                              {label}
                            </td>
                            <td className="py-0.5 pr-2">
                              {synced ? (
                                ...
                                <span className="text-[hsl(var(--win98-button-text)/0.4)]">—</span>
                              )}
                            </td>
                            <td className="py-0.5 text-right text-[hsl(var(--win98-button-text)/0.6)]">
                              {lastSync
          ...
          {activeTab === "storage" && (
            <div className="space-y-3">
              ...
              <fieldset className="border border-[hsl(var(--win98-shadow))] p-2 rounded-sm">
                ...
                <p className="text-[9px] text-[hsl(var(--win98-button-text)/0.5)] mt-1.5 text-center">
        ...
        {/* Status bar */}
        <div className="win98-sunken flex items-center gap-2 px-2 py-0.5 text-[10px] text-[hsl(var(--win98-button-text)/0.7)] border-t border-[hsl(var(--win98-shadow))]">
          {isOffline ? (
            <><WifiOff className="h-3 w-3" /> Offline — using cached data</>
          ) : (
