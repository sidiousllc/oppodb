...
export function SubjectAIPanel({ subjectType, subjectRef, subjectTitle, defaultScope = "national", defaultScopeRef = "" }: SubjectAIPanelProps) {
  ...
  const [audAnalysis, setAudAnalysis] = useState<any>(null);

  const [impactLoading, setImpactLoading] = useState(false);
  const [impactScope, setImpactScope] = useState(defaultScope);
  const [impactScopeRef, setImpactScopeRef] = useState(defaultScopeRef);
  ...
  async function genTalking() {
    setTpLoading(true);
    try {
      const { data, error } = await supabase.functions.invoke("subject-talking-points", {
        body: { subject_type: subjectType, subject_ref: subjectRef, audience, angle, tone, length, count, include_evidence: true, include_sections: sections, custom_instructions: customInstr, model },
      });
  ...
  async function genAudience(force = false) {
    setAudLoading(true);
    try {
      const { data, error } = await supabase.functions.invoke("subject-audience-analysis", {
        body: { subject_type: subjectType, subject_ref: subjectRef, force_refresh: force, include_sections: sections, model },
      });
      if (error) throw error;
      if (data?.error) throw new Error(data.error);
      setAudAnalysis(data.analysis);
  ...
  async function genImpact(force = false) {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("subject-impact-analysis", {
        body: { subject_type: subjectType, subject_ref: subjectRef, scope: impactScope, scope_ref: impactScopeRef || null, force_refresh: force, include_sections: sections, model },
      });
      if (error) throw error;
      if (data?.error) throw new Error(data.error);
    ...
    <div className="win98-raised bg-[hsl(var(--win98-face))] p-2 mt-3">
      <div className="flex items-center justify-between mb-2">
        <div className="text-[11px] font-bold flex items-center gap-1">
          <Sparkles className="h-3 w-3" /> AI Tools — {subjectTitle}
        </div>
          ...
          {audAnalysis && (
            <div className="space-y-2 text-[10px]">
              <div className="bg-white border border-[hsl(var(--win98-shadow))] p-2">
                <div className="font-bold mb-1">Effectiveness: {Math.round(audAnalysis.effectiveness_score)}/100</div>
                <p className="opacity-80">{audAnalysis.summary}</p>
              </div>
              {audAnalysis.audience_scores && (
                <div className="grid grid-cols-3 gap-1">
                  {Object.entries(audAnalysis.audience_scores).map(([k, v]: any) => (
              ...
              {(audAnalysis.segment_breakdown || []).length > 0 && (
                <div>
                  <div className="font-bold mb-1">Segment breakdown</div>
                  {audAnalysis.segment_breakdown.map((s: any, i: number) => (
              ...
              {(audAnalysis.risks || []).length > 0 && (
                <div>
                  <div className="font-bold mb-1">Risks</div>
                  {audAnalysis.risks.map((r: any, i: number) => (
      ...
      {tab === "impact" && (
        <div>
          <div className="flex gap-1 mb-2 items-center flex-wrap">
            <select value={impactScope} onChange={(e) => setImpactScope(e.target.value as any)} className="win98-input text-[10px] px-1 py-0.5">
