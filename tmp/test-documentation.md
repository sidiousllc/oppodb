...
## Test File
**Location:** `src/test/mnfinance-ssrf.test.ts`  
**To install:** Copy `tmp/test-mn-finance.ts` to `src/test/mnfinance-ssrf.test.ts`
...
## Expected Results
All 50 tests should pass, confirming that:
...
1. Copy the test file from tmp to the test directory:
   ```bash
...
- Tests: `src/test/mnfinance-ssrf.test.ts` (to be created from `tmp/test-mn-finance.ts`)
