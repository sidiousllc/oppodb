...
async function buildContext(admin: SAdmin, subject: any, selected: Section[], scope: string, scopeRef: string | null) {
...
serve(async (req) => {
  ...
  try {
    ...
    const { subject_type, subject_ref, scope = "national", scope_ref = null, force_refresh, include_sections = SECTIONS, model = "google/gemini-2.5-pro" } = await req.json();
